...
from constants import SUPPORTED_COUNTRIES, BOT_PERSONAS
...
LLM_MODEL_ID = os.environ.get(
    'LLM_MODEL_ID',
    'anthropic.claude-3-7-sonnet-20250219-v1:0'   # ← default to 3.7 Sonnet
)
...
# === ADD: KB identity/logging on cold start ===

# === ADD: minimal KB wiring log ===
try:
    ag = boto3.client("bedrock-agent", region_name=lambda_region)
    kb = ag.get_knowledge_base(knowledgeBaseId=KNOWLEDGE_BASE_ID)["knowledgeBase"]
    logger.info("KB wiring: id=%s name=%s status=%s",
                kb.get("knowledgeBaseId"), kb.get("name"), kb.get("status"))
    ds_list = ag.list_data_sources(knowledgeBaseId=KNOWLEDGE_BASE_ID).get("dataSources", [])
    for d in ds_list:
        ds = ag.get_data_source(knowledgeBaseId=KNOWLEDGE_BASE_ID, dataSourceId=d["dataSourceId"])["dataSource"]
        s3 = ds.get("dataSourceConfiguration", {}).get("s3Configuration", {})
        logger.info("KB DS: id=%s name=%s bucket=%s status=%s",
                    ds.get("dataSourceId"), ds.get("name"), s3.get("bucketArn"), ds.get("status"))
except Exception as e:
    logger.warning(f"KB wiring log failed: {e}")
# === END ADD ===
...
def knowledge_base_retrieval(prompt, kb_id, number_of_results=5, filter_dict=None): # Added number_of_results param
    ...
    try:
        ...
        # >>> ADD: explicit runtime call marker
        logger.info(">>> Calling Bedrock Agent Runtime.retrieve for KB %s (region=%s)", kb_id, lambda_region)
        # <<< END ADD (pre-call)
        ...
        kb_response = agent_runtime_client.retrieve(
            ...
            retrievalConfiguration=config
                ...
                # === ADD: minimal results log ===
        try:
            rr = kb_response.get('retrievalResults', [])
            uris = [r.get('location', {}).get('s3Location', {}).get('uri') for r in rr[:3]]
            buckets = sorted({(u.split('/')[2] if u and u.startswith('s3://') else None) for u in uris if u})
            logger.info("KB returned %d chunk(s). Top URIs=%s | buckets=%s", len(rr), uris, buckets)
        except Exception as e:
            logger.warning(f"KB result logging failed: {e}")
        # === END ADD ===
