...
export class CdkBackendStackInstanceC extends cdk.Stack {
  constructor(scope: Construct, id: string, props: CdkBackendStackProps) {
    ...
    const kb = new bedrock.KnowledgeBase(this, 'kb-instanceC', {
      ...
      instruction: 'Knowledge base for i2i chatbot', // Optional
    });
  ...
  const bucketC = new s3.Bucket(this, 'bucket-instanceC', {
    removalPolicy: cdk.RemovalPolicy.RETAIN,      // Keeps the bucket if you delete the stack
    autoDeleteObjects: false,                      // Protects existing objects from accidental deletion
    encryption: s3.BucketEncryption.S3_MANAGED,    // Optional but recommended
    versioned: false                                // Optional; enable if you want version history
  });

    // --- Knowledge Base Data Source ---
    // This associates the S3 bucket with the Knowledge Base via CDK.
     const dataSourceC = new bedrock.S3DataSource(this, 'datasource-instanceC', {
       bucket: bucketC,
       knowledgeBase: kb,
    ...
    const lambdaXbedrock = new lambda.Function(this, 'lambda-bedrock-instanceC', {
      ...
      environment: {
        ...
        S3_BUCKET_NAME: bucketC.bucketName, // Pass the actual bucket name
  ...
  conditions: {
    ...
    lambdaXbedrock.addToRolePolicy(new iam.PolicyStatement({
        ...
        resources: [bucketC.arnForObjects("*")], // Grant for all objects within the specific bucket
    }));
    ...
    webSocketApi.addRoute('sendMessage', { // Route key defined by client sending {"action": "sendMessage", ...}
      integration: new apigatewayv2_integrations.WebSocketLambdaIntegration(
        'ws-sendMessage-integration-instanceC', // Integration name
    ...
    const amplifyApp = new amplify.App(this, 'i2iC-ReactApp', { // Logical ID in CDK, affects Amplify App name
      appName: 'i2iC-ReactApp', // Optional: Explicit Amplify App name
